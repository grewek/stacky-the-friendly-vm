...
void stacky_bce_read_byte_code_from_disk(Stacky *stacky, const char *file_path) {
  ...
  size_t read_instruction_count = 0;
  fread(&read_instruction_count, sizeof(size_t), 1, input_file_handle);

  assert(read_instruction_count > 0);

  size_t instruction_count = read_instruction_count / sizeof(StackyInstruction);
  ...
  size_t remaining_data_segment_bytes = file_size - read_instruction_count;
  stacky->data_segment_size = fread(stacky->data_segment, sizeof(uint8_t), remaining_data_segment_bytes, input_file_handle);
 ...
 */
void stacky_bce_write_byte_code_to_disk(StackyInstruction *instructions, size_t code_segment_size, uint8_t *data_segment, size_t data_segment_size, const char *file_path) {
  assert(file_path != NULL);
  ...
  size_t instruction_count = (code_segment_size * sizeof(StackyInstruction)) + sizeof(size_t);
  fwrite(&instruction_count, sizeof(size_t), 1, output_file_handle);

  size_t bytes_written_code = fwrite(instructions, sizeof(StackyInstruction), code_segment_size, output_file_handle);
  if(bytes_written_code < code_segment_size) {
    fprintf(stderr, "stacky_byte_code_engine error: cannot write bytes to file %s: %s\n", file_path, strerror(errno));
    fclose(output_file_handle);
    exit(1);
  }

  size_t bytes_written_data = fwrite(data_segment, sizeof(uint8_t), data_segment_size, output_file_handle);
  if(bytes_written_data < data_segment_size) {
    fprintf(stderr, "stacky_byte_code_engine error: cannot write bytes to file %s: %s\n", file_path, strerror(errno));
    fclose(output_file_handle);
    exit(1);
  }
...
int main(int argc, char **argv) {
  ...
  if(strcmp(argv[1], "compile") == 0) {
    ...
    stacky_bce_write_byte_code_to_disk(stacky_vm.code_segment, stacky_vm.code_segment_size, stacky_vm.data_segment, stacky_vm.data_segment_size, argv[3]);
